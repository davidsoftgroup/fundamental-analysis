...
def make_period_options(fiscal_month, fiscal_day):
    ...
    for ptype in [3, 6, 9, 12]:
        ...
        label = "{} {} {}".format(names[ptype], end_d, month_name)
        options[label] = ptype
    return options
...
def period_label(period_type, end_day=None, end_month=None):
    ...
    if end_day and end_month:
        ...
        return "{} {} {}".format(base, end_day, month_name)
    return base
...
company_options = {}
for c in companies:
    label = "{} - {} (پایان سال مالی: {} {})".format(
        c["symbol"],
        c["name_fa"] or "",
        c["fiscal_end_day"],
        MONTH_NAMES.get(c["fiscal_end_month"], "")
    )
    company_options[label] = c

placeholder = "— یک شرکت انتخاب کنید —"
all_labels = [placeholder] + list(company_options.keys())

selected_label = st.selectbox("انتخاب شرکت:", options=all_labels)

# اگر هنوز انتخاب نشده
if selected_label == placeholder or selected_label not in company_options:
    st.info("برای ورود اطلاعات، ابتدا یک شرکت از لیست بالا انتخاب کنید.")
    st.stop()
...
st.caption("شرکت انتخاب‌شده: **{}**".format(selected_company["symbol"]))
st.markdown("---")
...
with st.form("financial_form"):
    ...
    with m1:
        st.metric("سود خالص عملیاتی", "{:,.0f}".format(op_profit) if op_profit is not None else "—")
    with m2:
        st.metric("حاشیه سود خالص", "{:.1f}%".format(net_margin * 100) if net_margin is not None else "—")
    with m3:
        st.metric("نسبت مطالبات به دارایی جاری", "{:.1f}%".format(recv_ratio * 100) if recv_ratio is not None else "—")
    with m4:
        st.metric("نسبت سود مصوب به جامع", "{:.1f}%".format(div_ratio * 100) if div_ratio is not None else "—")
    ...
    if submitted:
        try:
            ...
            if existing:
                ...
                st.success("صورت مالی به‌روزرسانی شد.")
            else:
                cursor.execute("""
            ...
            st.error("خطا در ثبت: {}".format(e))

    if delete_btn:
        if not confirm_delete:
            st.warning("برای حذف، ابتدا گزینه تأیید را تیک بزنید.")
        else:
            try:
                conn = get_connection()
                cursor = conn.cursor()
...
if periods:
    ...
    for row in periods:
        ...
        data.append({
            ...
            "درآمد عملیاتی": "{:,.0f}".format(row["operating_revenue"]) if row["operating_revenue"] is not None else "—",
            "سود خالص عملیاتی": "{:,.0f}".format(op_p) if op_p is not None else "—",
            "حاشیه سود": "{:.1f}%".format(margin * 100) if margin is not None else "—",
            "نسبت مطالبات": "{:.1f}%".format(recv_r * 100) if recv_r is not None else "—",
        })
    ...
    with load_col1:
        period_labels = [
            "{} - {}".format(r["year_solar"], period_label(r["period_type"], r["end_day"], r["end_month"]))
            for r in periods
        ]
        load_choice = st.selectbox("انتخاب دوره:", options=period_labels, key="load_period")
    with load_col2:
        st.write("")
        st.write("")
        if st.button("بارگذاری"):
            idx = period_labels.index(load_choice)
            row = periods[idx]
            conn = get_connection()
            full = conn.execute("""
            ...
            if full:
                ...
                st.success("اطلاعات بارگذاری شد.")
                st.experimental_rerun()
else:
